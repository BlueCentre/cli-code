...
async def run_tool_directly(
    integration: MCPToolIntegration, tool_name: str, parameters: Dict[str, Any], console: Console
):
    """
    ...
    try:
        ...
        tool_call = {"id": tool_call_id, "type": "function", "function": {"name": tool_name, "arguments": parameters}}
...
async def main():
    ...
    run_parser.add_argument(
        "--parameter-file", type=str, help="Path to a JSON file containing parameters to pass to the tool"
    )
    ...
    client = MCPClient(endpoint=endpoint, api_key=api_key, model=os.environ.get("MCP_MODEL", "default"))
