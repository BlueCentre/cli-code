...
from mcp_code.mcp_client.messages.error_codes import get_error_message, is_retryable_error
from mcp_code.mcp_client.messages.exceptions import NonRetryableError, RetryableError
...
async def send_message(
    ...
    for attempt in range(1, retries + 1):
        ...
        try:
            ...
            last_exception = exc
            logging.error(
                f"[send_message] Timeout waiting for response to method '{method}' (Attempt {attempt}/{retries})"
            )
            if is_final_attempt:
                raise
            ...
            last_exception = exc
            logging.error(
                f"[send_message] Retryable error during '{method}' request: {exc} (Attempt {attempt}/{retries})"
            )
            if is_final_attempt:
            ...
            last_exception = exc
            logging.error(f"[send_message] Error during '{method}' request: {exc} (Attempt {attempt}/{retries})")
            if is_final_attempt:
                raise
    ...
    while True:
        ...
        if response.error is not None:
            if response.result is not None:
                logging.warning(f"[send_message] Invalid JSON-RPC response: contains both result and error")

            error_data = response.error
            # Per JSON-RPC 2.0 spec, error codes must be integers
            error_code = error_data.get("code", -32603)  # Internal error if no code
            error_msg = (
                f"JSON-RPC Error: {error_data.get('message', get_error_message(error_code))} (code: {error_code})"
            )
