...
async def test_send_prompts_list():
    ...
    sample_prompts = {
        "prompts": [
            ...
            },
            {
                ...
                "arguments": [{"name": "location", "description": "The location to get weather for", "required": True}],
            },
        ],
    ...
    async with anyio.create_task_group() as tg:
        ...
        # Client side request
        result = await send_prompts_list(read_stream=read_receive, write_stream=write_send, cursor="test-cursor")
...
async def test_send_prompts_get():
    ...
    sample_prompt_result = {
        ...
        "messages": [{"role": "user", "content": {"type": "text", "text": "Provide weather information for New York"}}],
    }
    ...
    async def server_task():
        try:
            ...
            response = JSONRPCMessage(
                id=req.id, error={"code": -32602, "message": "Invalid params: Prompt not found: invalid_prompt_name"}
            )
            await read_send.send(response)
        except Exception as e:
            pytest.fail(f"Server task failed: {e}")
