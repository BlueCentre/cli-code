...
async def test_send_ping_error_response():
    ...
    async def error_server():
        try:
            ...
            # Send error response
            response = JSONRPCMessage(id=req.id, error={"code": 500, "message": "Internal server error"})
            await read_send.send(response)
        except Exception as e:
...
async def test_send_ping_timeout():
    ...
    async with anyio.create_task_group() as tg:
        ...
        result = await send_ping(read_stream=read_receive, write_stream=write_send, timeout=0.5, retries=1)
