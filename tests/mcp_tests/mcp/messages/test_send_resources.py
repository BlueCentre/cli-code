...
from mcp_code.mcp_client.messages.exceptions import NonRetryableError
...
from mcp_code.mcp_client.messages.resources.send_messages import (
    send_resources_list,
    send_resources_read,
...
async def test_send_resources_list():
    ...
    async with anyio.create_task_group() as tg:
        ...
        # Client side request
        result = await send_resources_list(read_stream=read_receive, write_stream=write_send, cursor="test-cursor")
...
async def test_send_resources_read():
    ...
    async with anyio.create_task_group() as tg:
        ...
        # Client side request
        result = await send_resources_read(read_stream=read_receive, write_stream=write_send, uri=test_uri)
...
async def test_send_resources_templates_list():
    ...
    async with anyio.create_task_group() as tg:
        ...
        # Client side request
        result = await send_resources_templates_list(read_stream=read_receive, write_stream=write_send)
...
async def test_send_resources_subscribe_success():
    ...
    async with anyio.create_task_group() as tg:
        ...
        # Client side request
        result = await send_resources_subscribe(read_stream=read_receive, write_stream=write_send, uri=test_uri)
    ...
    async def server_task():
        try:
            ...
            response = JSONRPCMessage(
                id=req.id, error={"code": -32002, "message": "Resource not found", "data": {"uri": test_uri}}
            )
            await read_send.send(response)
        except Exception as e:
            pytest.fail(f"Server task failed: {e}")
...
async def test_resources_list_error_handling():
    ...
    async def server_task():
        try:
            ...
            # Send error response
            response = JSONRPCMessage(id=req.id, error={"code": -32603, "message": "Internal server error"})
            await read_send.send(response)
        except Exception as e:
            pytest.fail(f"Server task failed: {e}")
