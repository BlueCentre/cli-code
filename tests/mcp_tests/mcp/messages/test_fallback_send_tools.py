...
async def test_send_tools_list_fallback(monkeypatch):
    ...
    # 2) Also remove any of your mcp_code modules that might be cached
    #    so that on reload, they see pydantic is missing.
    #    (Remove whichever submodules are relevant – e.g. mcp_code.mcp_client, mcp_code.mcp_client.messages, etc.)
    for mod_name in list(sys.modules):
        if mod_name.startswith("mcp_code.mcp_client"):
            del sys.modules[mod_name]

    # 3) Re-import the modules under test
    from anyio.streams.memory import MemoryObjectReceiveStream, MemoryObjectSendStream

    import mcp_code.mcp_client.messages.json_rpc_message  # triggers fallback
    import mcp_code.mcp_client.messages.tools.send_messages as tools_messages
