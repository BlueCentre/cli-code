...
from mcp_code.mcp_client.messages.tools.send_messages import send_tools_call, send_tools_list
...
async def test_send_tools_list():
    ...
    async with anyio.create_task_group() as tg:
        ...
        # Client side request
        result = await send_tools_list(read_stream=read_receive, write_stream=write_send, cursor="test-cursor")
...
async def test_send_tools_call():
    ...
    async with anyio.create_task_group() as tg:
        ...
        result = await send_tools_call(
            read_stream=read_receive, write_stream=write_send, name=test_tool, arguments=test_args
        )
...
async def test_send_tools_call_protocol_error():
    ...
    async def server_task():
        try:
            ...
            response = JSONRPCMessage(id=req.id, error={"code": -32602, "message": "Unknown tool: invalid_tool_name"})
            await read_send.send(response)
        except Exception as e:
            pytest.fail(f"Server task failed: {e}")
