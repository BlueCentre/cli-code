...
from mcp_code.mcp_client.messages.prompts.send_messages import send_prompts_get, send_prompts_list
...
async def test_send_prompts_get_multi_message_types():
    ...
    complex_prompt_result = {
        ...
        "messages": [
            {
                "role": "user",
                "content": {"type": "text", "text": "Analyze this code sample and explain the image below:"},
            },
            {
                "role": "user",
                "content": {
            ...
            {"role": "user", "content": {"type": "image", "data": sample_base64_image, "mimeType": "image/png"}},
            {
                "role": "assistant",
                "content": {"type": "text", "text": "The code defines a simple function that prints 'Hello, world!'."},
            },
        ],
    }
    ...
    async with anyio.create_task_group() as tg:
        ...
        result = await send_prompts_get(
            read_stream=read_receive, write_stream=write_send, name=test_prompt_name, arguments=test_prompt_args
        )
    ...
    async def server_task():
        try:
            ...
            response = JSONRPCMessage(
                id=req.id, error={"code": -32602, "message": "Invalid params: Missing required argument 'code'"}
            )
            await read_send.send(response)
        except Exception as e:
            pytest.fail(f"Server task failed: {e}")
...
async def test_send_prompts_get_resource_blob():
    ...
    resource_prompt_result = {
        ...
        "messages": [
            {"role": "user", "content": {"type": "text", "text": "Here's an example binary file:"}},
            {
                "role": "user",
                "content": {
                    "type": "resource",
    ...
    async with anyio.create_task_group() as tg:
        ...
        # Client side request
        result = await send_prompts_get(read_stream=read_receive, write_stream=write_send, name="binary_example")
...
async def test_send_prompts_get_server_resource_error():
    ...
    resource_error_result = {
        "isError": True,
        "content": [{"type": "text", "text": "Error accessing resource: Resource not found or access denied"}],
    }
...
async def test_send_prompts_list_extensive():
    ...
    extensive_prompts = {
        "prompts": [
            ...
            },
            {
                ...
                "arguments": [
                    {"name": "imageData", "description": "Base64-encoded image data", "required": True},
                    {
                        "name": "format",
    ...
    async with anyio.create_task_group() as tg:
        ...
        # Client side request
        result = await send_prompts_list(read_stream=read_receive, write_stream=write_send)
