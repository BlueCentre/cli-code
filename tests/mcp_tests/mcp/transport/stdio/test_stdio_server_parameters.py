...
try:
    ...
    from mcp_code.mcp_client.mcp_pydantic_base import ValidationError
...
def test_stdio_server_parameters_creation():
    ...
    params = StdioServerParameters(command="python", args=["-m", "mcp.server"], env={"TEST_ENV": "value"})
    assert params.command == "python"
...
def test_stdio_server_parameters_model_dump():
    ...
    assert dump == {"command": "python", "args": ["-m", "mcp.server"], "env": {"TEST_ENV": "value"}}
