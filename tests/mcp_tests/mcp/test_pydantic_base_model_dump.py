...
from mcp_code.mcp_client.mcp_pydantic_base import PYDANTIC_AVAILABLE, ConfigDict, Field, McpPydanticBase
...
def test_custom_json_options():
    ...
    # Create model with wrapper
    model = create_model(ComplexModel, name="complex", data={"a": 1, "b": [1, 2, 3]})
...
def test_nested_models():
    ...
    parent = create_model(ParentModel, name="parent", child=child)
...
def test_arbitrary_fields():
    ...
    if PYDANTIC_AVAILABLE:
        ...
        model = create_model(ArbitraryModel, command="test", args=["foo", "bar"], extra={"a": 1})
    else:
        # For fallback, we can use McpPydanticBase directly
        model = create_model(McpPydanticBase, command="test", args=["foo", "bar"], extra={"a": 1})
...
def test_complex_types():
    ...
    model = create_model(
        ComplexTypesModel,
        integers=[1, 2, 3, 4, 5],
        nested_dict={"outer": {"inner1": 10, "inner2": 20}},
        mixed=[
            {"type": "a", "value": 1},
            {"type": "b", "value": "string"},
            {"type": "c", "value": [1, 2, 3]},
