...
class TestMCPToolIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.tool_service.execute_tool = AsyncMock(return_value={
            "name": "test_tool",
    ...
    async def test_handle_tool_call(self):
        ...
        self.tool_service.execute_tool.assert_called_once_with(
            ...
            tool_calls=[
                {
                    ...
                    "function": {
                        "name": "test_tool",
                        "arguments": json.dumps({"param1": "test_value_1"})
                    }
                },
                {
                    "id": "test_id_2",
                    ...
                    "function": {
                        "name": "test_tool",
                        "arguments": json.dumps({"param1": "test_value_2"})
                    }
                }
            ]
        )
    ...
    async def test_execute_conversation_turn_without_tool_calls(self):
        ...
        response, history = await self.integration.execute_conversation_turn(
            "User message",
            []
        )
    ...
    async def test_execute_conversation_turn_with_history(self):
        ...
        # Execute a conversation turn
        response, new_history = await self.integration.execute_conversation_turn(
            "User message",
            history
        )
