...
from cli_code.main import start_interactive_session, cli, show_help, console
...
try:
    ...
    pytest.skip("Missing optional dependencies (like click), skipping integration tests for main.", allow_module_level=True)
...
class TestMainErrorHandling:
    ...
    def test_cli_with_missing_model(self):
        ...
        result = self.runner.invoke(cli, [])
        assert result.exit_code == 1
        self.mock_console.print.assert_any_call("[bold red]Error:[/bold red] No default model configured for provider 'gemini' and no model specified with --model.")
...
class TestInteractiveSession:
    ...
    def test_interactive_session_with_missing_credential(self):
        ...
        assert any("Gemini API Key not found" in args_str for args_str in call_args_list), "Missing credential error not printed"
    ...
    def test_interactive_session_with_model_initialization_error(self):
        ...
        with patch('cli_code.main.GeminiModel', side_effect=Exception("Init Error")):
            ...
            call_args_list = [str(args[0]) for args, kwargs in self.mock_console.print.call_args_list if args]
            assert any("Error initializing model 'gemini-pro'" in args_str and "Init Error" in args_str for args_str in call_args_list), \
    ...
    def test_context_initialization_with_rules_dir(self):
        ...
        call_args_list = [str(args[0]) for args, kwargs in self.mock_console.print.call_args_list if args]
        assert any("Context will be initialized from 2 .rules/*.md files." in args_str for args_str in call_args_list), \
    ...
    def test_interactive_session_interactions(self):
        ...
        mock_agent.generate.side_effect = generate_sequence([
            "Response 1", "Response 2 (for /custom)", "Response 3",
        ])
        self.mock_console.input.side_effect = [
            "Hello", "/custom", "Empty input", "/exit"
        ]
        ...
        with patch('cli_code.main.Markdown') as mock_markdown_local:
            ...
            response_prints = [args[0] for args, kwargs in print_calls if args and "Mocked Markdown Instance" in str(args[0])]
