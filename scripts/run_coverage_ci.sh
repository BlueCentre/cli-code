...
for test_file in \
  ...
  echo "Running $test_file with timeout $CI_TIMEOUT seconds..."
...
# In CI, we should fail if there are failing tests
if [ -n "$CI" ] && [ $FAILED_TESTS -gt 0 -o $TIMED_OUT_TESTS -gt 0 ]; then
  echo "Test run failed with $FAILED_TESTS failing tests and $TIMED_OUT_TESTS timed out tests" | tee -a "$SUMMARY_LOG"
  exit 1
fi
